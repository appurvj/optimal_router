...
$('#indiv-steps').height($('#time-info').height());
...
function activateMode(str){
	...
	if(selectedMode != "TRANSIT"){
		$('#time-info').hide();
		$('#addrs-info').removeClass('col-md-8 col-xs-7').addClass('col-md-12 col-xs-12');
	}
	else{
		$('#time-info').show();
		$('#addrs-info').removeClass('col-md-12 col-xs-12').addClass('col-md-8 col-xs-7');
	}
	if(submittedOnce)
		runCalc();
...
function GetDate(){
	...
	var amPM = $('#stime').val().split(" ")[1];
    hrs_mins = $("#stime").val().split(" ")[0].split(":");
    hrs_mins[0] = parseInt(hrs_mins[0]);
    hrs_mins[1] = parseInt(hrs_mins[1]);
    if(amPM=="PM")
    	hrs_mins[0] += 12;
...
function GetAddressList(){
  ...
  function combineResults(){
    ...
    var reqKey = Object.keys(resultList[0])[2];
    ...
    result[reqKey] = {destination: addressList[addressList.length - 1],
